	...
	L::Target: LdkLogger,
{
	...
	pub(crate) async fn lsps2_receive_to_jit_channel(
		...
		self.lsps2_receive_to_jit_channel_inner(amount_msat, description, expiry_secs, max_total_lsp_fee_limit_msat, None, min_final_cltv_expiry_delta).await
	}

	pub(crate) async fn lsps2_receive_to_jit_channel_for_hash(
		&self, amount_msat: u64, description: &Bolt11InvoiceDescription, expiry_secs: u32,
		max_total_lsp_fee_limit_msat: Option<u64>, payment_hash: PaymentHash, min_final_cltv_expiry_delta: Option<u16>,
	) -> Result<(Bolt11Invoice, u64), Error> {
		self.lsps2_receive_to_jit_channel_inner(amount_msat, description, expiry_secs, max_total_lsp_fee_limit_msat, Some(payment_hash), min_final_cltv_expiry_delta).await
	}

	async fn lsps2_receive_to_jit_channel_inner(
		&self, amount_msat: u64, description: &Bolt11InvoiceDescription, expiry_secs: u32,
		max_total_lsp_fee_limit_msat: Option<u64>, manual_claim_payment_hash: Option<PaymentHash>,
	...
	pub(crate) async fn lsps2_receive_variable_amount_to_jit_channel(
		...
		self.lsps2_receive_variable_amount_to_jit_channel_inner(description, expiry_secs, max_proportional_lsp_fee_limit_ppm_msat, None, min_final_cltv_expiry_delta).await
	}

	pub(crate) async fn lsps2_receive_variable_amount_to_jit_channel_for_hash(
		&self, description: &Bolt11InvoiceDescription, expiry_secs: u32,
		max_proportional_lsp_fee_limit_ppm_msat: Option<u64>, payment_hash: PaymentHash, min_final_cltv_expiry_delta: Option<u16>,
	) -> Result<(Bolt11Invoice, u64), Error> {
		self.lsps2_receive_variable_amount_to_jit_channel_inner(description, expiry_secs, max_proportional_lsp_fee_limit_ppm_msat, Some(payment_hash), min_final_cltv_expiry_delta).await
	}

	async fn lsps2_receive_variable_amount_to_jit_channel_inner(
		&self, description: &Bolt11InvoiceDescription, expiry_secs: u32,
		max_proportional_lsp_fee_limit_ppm_msat: Option<u64>, manual_claim_payment_hash: Option<PaymentHash>,
		min_final_cltv_expiry_delta: Option<u16>,
	) -> Result<(Bolt11Invoice, u64), Error> {
		let fee_response = self.lsps2_request_opening_fee_params().await?;
		...
		let invoice =
			self.lsps2_create_jit_invoice(buy_response, None, description, expiry_secs, manual_claim_payment_hash, min_final_cltv_expiry_delta)?;
	...
	fn lsps2_create_jit_invoice(
		...
		// Use provided CLTV delta or default (LSPS2 requires at least 2 more than usual)
		let cltv_delta = min_final_cltv_expiry_delta.unwrap_or(MIN_FINAL_CLTV_EXPIRY_DELTA + 2);
		
		let (payment_hash, payment_secret) = if let Some(manual_hash) = manual_claim_payment_hash {
			// Manual claim: use create_inbound_payment_for_hash
			let payment_secret = self.channel_manager
				...
				.map_err(|e| {
					log_error!(self.logger, "Failed to register inbound payment for hash: {:?}", e);
					Error::InvoiceCreationFailed
				})?;
			(manual_hash, payment_secret)
		} else {
			// Auto-claim: use create_inbound_payment
			self.channel_manager
				.create_inbound_payment(None, expiry_secs, Some(cltv_delta))
				.map_err(|e| {
